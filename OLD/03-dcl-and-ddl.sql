...
-- 사용자 생성
CREATE USER C##NAMGUNG IDENTIFIED BY 1234;
-- 비밀번호 변경
ALTER USER C##NAMGUNG IDENTIFIED BY test;
-- 사용자 삭제
DROP USER C##NAMGUNG;
-- 경우에 따라 내부에 테이블 등 데이터베이스 객체가 생성된 사용자
DROP USER C##NAMGUNG CASCADE; -- 폭포수
...
-- 사용자 계정에게 접속 권한 부여
GRANT create session TO C##NAMGUNG;
-- 일반적으로 데이터베이스 접속, 테이블 만들어 사용하려면
-- CONNECT, RESOURCE 룰을 부여
GRANT connect, resource TO C##NAMGUNG;
-- Oracle 12이상에서는 사용자 테이블 스페이스에 공간 부여 필요
ALTER USER C##NAMGUNG DEFAULT TABLESPACE USERS QUOTA unlimited ON USERS;
...
GRANT select ON hr.employees TO C##NAMGUNG;
GRANT select ON hr.departments TO C##NAMGUNG;
...
REVOKE select ON hr.departments FROM C##NAMGUNG;
